...
enum LongSessionReminderNotification {
    ...
    static func register(on center: UNUserNotificationCenter) {
        center.setNotificationCategories([UNNotificationCategory(identifier: category, actions: [
            UNNotificationAction(identifier: clockOut, title: String(localized: "Clock out", bundle: .app), options: []),
            UNNotificationAction(identifier: setEndTime, title: String(localized: "Set end time", bundle: .app), options: [.foreground]),
            UNNotificationAction(identifier: snooze, title: String(localized: "Remind in 1 hour", bundle: .app), options: [])
...
final class LongSessionReminderController: ObservableObject {
    ...
    private func drain() async {
        ...
        while processed != revision {
            ...
            content.title = String(localized: "Still working?", bundle: .app)
            content.body = String(localized: "Clockin has been running for \(DurationText.compact(running.elapsed(at: date))).", bundle: .app)
            ...
            do {
                ...
                guard processed == revision else { continue }
                errorMessage = String(localized: "Could not schedule reminder: \(error.localizedDescription)", bundle: .app)
            }
        }
    }
