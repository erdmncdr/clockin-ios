...
check(level.pending.isEmpty, "lower level does not celebrate")
check(level.lastLevel == 56, "a level taken back is forgotten")
level.ingest(newer, now: 4, canReact: false)
check(level.pending == [.levelUp(level: 60, hours: 270)], "a level earned again celebrates again")

var cancelled = CelebrationQueue(lastLevel: 56, seenBadgeIDs: Set(archive.badges.map(\.id)))
cancelled.ingest(newer, now: 0, canReact: false)
check(cancelled.pending == [.levelUp(level: 60, hours: 270)], "session crosses a level")
cancelled.ingest(archive, now: 1, canReact: false)
check(cancelled.pending.isEmpty, "a cancelled session withdraws its queued level")
check(cancelled.lastLevel == 56, "withdrawn level is not remembered")
